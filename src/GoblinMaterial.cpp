...
Material* SubsurfaceMaterialCreator::create(const ParamSet& params,
    ...
    if (params.hasVector3("Kd")) {
        Vector3 Kd = params.getVector3("Kd");
        Vector3 diffuseMeanFreePath = params.getVector3("mean_free_path",
            Vector3(1.0f, 1.0f, 1.0f));
        return new SubsurfaceMaterial(
			Color(Kd[0], Kd[1], Kd[2]),
			Color(diffuseMeanFreePath[0], diffuseMeanFreePath[1], diffuseMeanFreePath[2]),
			Kr, index, g, bump);
    } else {
